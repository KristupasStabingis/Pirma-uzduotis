...
void nuskaitymas(vector<studentas>& stud, int kiekis);
...
int main()
{
	...
	int vargs = 0;
	int kiet = 0;
		cout << "Ar norite dirbti su failu, ar pazymius vesti pats konsoleje? Su failu - spauskite F, pazymius vesti - R" << endl;
		cin >> simbolis;
		if (simbolis != "F" && simbolis != "f" && simbolis != "R" && simbolis != "r") {
			cout << "." << endl;
		}
		else if (simbolis == "F" || simbolis == "f") {
			...
			isvedimas << setw(20) << left << "Vardas" << setw(20) << left << "Pavarde" << setw(20) << left << "Galutinis_balas";
			...
			if (simbolis != "M" && simbolis != "m" && simbolis != "V" && simbolis != "v") {
				...
				cout << "Kiek pazymiu generuojame?" << endl;
				cin >> n;
				if (n < 0) {
					...
					cout << "Failo su " + to_string(kiekis) + " studentu/-ais kurimas uztruko: " << diff.count() << endl;
				...
				if (n < 0) {
					...
					auto end = high_resolution_clock::now();
					std::chrono::duration<double> diff = end - start;
					cout << "Failo su " + std::to_string(kiekis) + " studentu/-ais kurimas uztruko: " << diff.count() << endl;
					cout << "Ar norite sukurti txt faila su pasirinktu kiekiu studentu ir suskirstymu i dvi kategorijas - vargsiukus ir kietiakus? Jei taip - spauskite T, jei ne - spauskite N" << endl;
					cin >> simbolis;
					if (simbolis != "T" && simbolis != "t" && simbolis != "N" && simbolis != "n") {
						cout << "Toks simbolis nera galimas, bandykite is naujo" << endl;
						...
						auto end = high_resolution_clock::now();
						std::chrono::duration<double> diff = end - start;
						cout << "Failo rusiavimas su " + to_string(kiekis) + " studentais i dvi grupes uztruko : " << diff.count() << " s\n";
						...
						for (int i = 0; i < kiekis; i++) {
							...
							float paz = 5.00;
							if (st.at(i).galutinis_balas < paz) {
								vargs_failas << st.at(i).vardas << setw(20) << st.at(i).pavarde << setw(20) << st.at(i).galutinis_balas << endl;
							}
						...
						for (int j = 0; j < kiekis; j++) {
							float paz = 5.00;
							if (st.at(j).galutinis_balas >= paz) {
								kiet_failas << st.at(j).vardas << setw(20) << st.at(j).pavarde << setw(20) << st.at(j).galutinis_balas << endl;
							}
						}
						auto end2 = high_resolution_clock::now();
						std::chrono::duration<double> diff2 = end2 - start2;
						...
						cout << "Spauskite bet koki mygtuka, kad iseitumete" << endl;
					}
