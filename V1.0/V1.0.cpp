...
double count_galutinis_balas_vidurkis(vector<int> paz) {
	studentas st;
...
double count_galutinis_balas_mediana(vector<int> paz) {
	studentas st;
...
double count_galutinis_balas_vidurkisLIST(vector<int> paz) {
	studentas student;
...
double count_galutinis_balas_medianaLIST(vector<int> paz) {
	studentas student;
	int vidurinis_skaicius;
	sort(paz.begin(), paz.end());
	vidurinis_skaicius = size(paz) / 2;
...
void erase(std::vector<int>& v, float key)
{
...
int main()
{
	...
	if (simbolis != "L" && simbolis != "l" && simbolis != "V" && simbolis != "v") {
		...
		cout << "Ar norite studentus dalyti/rusiuoti 1 strategija ar 2 strategija. 1 strategija - 1, 2 strategija - 2?" << endl;
		cout << "1 strategija: Bendro studentai konteinerio(vector ir list tipu) skaidymas(rusiavimas) i du naujus to paties tipo konteinerius : vargsiuku ir kietiaku.Tokiu budu tas pats studentas yra dvejuose konteineriuose : bendrame studentai ir viename is suskaidytu(vargsiukai arba kietiakai)." << endl;
		cout << "2 strategija: Bendro studentų konteinerio (vector ir list) skaidymas (rusiavimas) panaudojant tik viena nauja konteineri: vargsiukai. Tokiu budu, jei studentas yra vargsiukas, ji turime ikelti i naujaji vargsiuku konteineri ir istrinti is bendro studentai konteinerio. Po sio zingsnio studentai konteineryje liks vien tik kietiakai." << endl;
		cin >> n;
		if (n != 1 && n != 2) {
			cout << "Pasirinkimas neimanomas, bandykite dar karta!";
		}
		else if (n == 1) {
			...
			if (simbolis != "T" && simbolis != "t" && simbolis != "N" && simbolis != "n") {
				...
				list<studentas>::iterator IT = student.begin();
				while (IT != student.end())
				{
					if (count_galutinis_balas_vidurkisLIST(paz) < 5.0 && count_galutinis_balas_medianaLIST(paz) < 5.0)
					{
				...
				std::ofstream IrasymasVargsai;
				IrasymasVargsai.open("vargsai" + std::to_string(kiekis) + ".txt");

				for (list<studentas>::iterator IT = vargsai.begin(); IT != vargsai.end(); IT++)
				{
					IrasymasVargsai << (*IT).vardas;
					IrasymasVargsai << (*IT).pavarde;

					std::streamsize prec = cout.precision();
					IrasymasVargsai << std::fixed << setprecision(2) << count_galutinis_balas_vidurkisLIST(paz) << " " << setprecision(prec) << endl;
				}
				for (auto i = 0; i != IlgiausiasVardas + IlgiausiaPavarde + 10; i++)
					IrasymasVargsai << "-";
				IrasymasVargsai << endl;
				IrasymasVargsai.close();


				std::ofstream IrasymasProtingi;
				IrasymasProtingi.open("Protingi" + std::to_string(kiekis) + ".txt");

				for (list<studentas>::iterator IT = student.begin(); IT != student.end(); IT++)
				{
					...
					std::streamsize prec = cout.precision();
					IrasymasProtingi << std::fixed << setprecision(2) << count_galutinis_balas_vidurkisLIST << " " << count_galutinis_balas_medianaLIST << setprecision(prec) << endl;
				}
				for (auto i = 0; i != IlgiausiasVardas + IlgiausiaPavarde + 10; i++)
					IrasymasProtingi << "-";
		...
		if (simbolis != "K" && simbolis != "k" && simbolis != "R" && simbolis != "r") {
			...
			if (n != 1 && n != 2) {
				...
				if (simbolis != "T" && simbolis != "t" && simbolis != "N" && simbolis != "n") {
					...
					IrasymasVargsiukai.open("Vargsiukai" + std::to_string(kiekis) + ".txt");
					for (int i = 0; i != kiekis; i++)
					{
						float paz = 5.00;
						if (st.at(i).galutinis_balas < paz) {
							IrasymasVargsiukai << st.at(i).vardas << setw(20);
							IrasymasVargsiukai << st.at(i).pavarde << setw(18);
							IrasymasVargsiukai << setprecision(3) << st.at(i).galutinis_balas << endl;
						}
					}

						/*std::ofstream IrasymasKietiakai;
						IrasymasKietiakai.open("Studentai_" + std::to_string(kiekis) + ".txt");

						for (int i = 0; i < kiekis; i++)
						{
							IrasymasKietiakai << st.at(i).vardas << setw(20) << st.at(i).pavarde << setw(18) << setprecision(3) << st.at(i).galutinis_balas << endl;
							IrasymasKietiakai.close();
						}*/
